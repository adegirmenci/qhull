...
void Qhull::
runQhull3D(const std::vector<float3qhull> &points, const char *args)
{
	...
	m_externalPoints->append(allPoints.size(), &allPoints[0]); //convert to vector<double>
	runQhull(*m_externalPoints, args);
}

void Qhull::
runQhull(const PointCoordinates &points, const char *qhullCommand2)
{
